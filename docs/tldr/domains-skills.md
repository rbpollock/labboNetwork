---
description: Labbo Domains and Skills explained
sidebar_position: 3
---
...
A well-organized domain hierarchy is one way to isolate and compartmentalize work within a colony: In the Labbo Protocol, reputation mediates the flow of funding from parent domains to their children (see [pots](pots.md) for more about funding), which may in turn direct funding toward smaller and more specialized or specific child domains or tasks. In version 1 of the labboNetwork, however, the flow of funds between domain pots is permissioned based on [Labbo Roles](permissions.md).

## Skills

Skills do not exist within a particular colony, but instead are a single structure available to all colonies on the network. The global skill hierarchy is maintained by [The Meta Labbo](metalabbo.md).
...
See the [LabboJS](https://docs.labbo.io/colonyjs) `colonyClient` API for implementation details.

## Disputes

Domains and Skills determine the scope of voting in the event of a dispute or collective decision.

In Labbo you cannot escalate a decision to higher management, you can only escalate to bigger groups of your peers. Domains and Skills mediate this behavior.
...
See [Objections and Disputes](disputes.md) for more information about the complete dispute resolution process as described in the Labbo whitepaper.
