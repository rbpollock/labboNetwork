...
In an organization of any type, reputation is an essential heuristic for people to keep track of each other's perceived merit in an environment of limited information. Reputation within Labbo is meant to stay close to the concept of reputation as it is commonly understood. Reputation is a representation of _merit_, which in Labbo implies an immutable record of contributions to a shared goal.
...
The Reputation System in Labbo is a tool for governance in a decentralized context: It allows for members of an organization to have different amounts of influence over the organization, without the use of a management hierarchy or frequent voting.
...
_It is important to note that in the first deployed version of the Labbo Network, reputation will play a more passive role in a labbo. Reputation will still determine payouts during the rewards cycle, but influence over domain/skill creation, funding tasks, and other important functions will instead be mediated by a permissioned_ [_authority role_](permissions.md)_._

## Gaining and Losing Reputation

Unlike a token, reputation cannot be transacted between accounts. Reputation can only be gained or lost through interactions with other members of a labbo.

### Completing Tasks

The most straightforward way of gaining reputation is by completing tasks within the labbo. Tasks funded with internal tokens generate reputation for their recipient upon payout.
...
After a colony is created, reputation can only be gained and lost through normal interactions within a labbo. But at the colony's beginning, we are presented with a bootstrapping problem: When a colony is new, no-one has yet completed any work in it and so nobody will have earned any reputation.
